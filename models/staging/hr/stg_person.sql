{{
    config(
            pre_hook=[ "create table if not exists {{ this }} (intcol int)",
      "truncate table {{ this }} "
    ],
        post_hook=[
...
select id,
       ...
       decode(marital_status,'W','Widowed','M','Married','S','Single') as marital_status,
       decode(gender,'M','Male','F','Female','U','Unspecified') as gender,
       start_date,
